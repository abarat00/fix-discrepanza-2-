...
Usage:
    py visualize_portfolio.py --results_dir results/baseline_full
...
def plot_portfolio_performance(test_data, training_data, config, results_dir):
    ...
    if test_data.get('portfolio_value_log') is not None:
        ...
        if len(portfolio_values) > 2 and portfolio_values[-1] < portfolio_values[-2] * 0.9:
            ...
            # Crea una copia modificata dell'array per il grafico
            portfolio_values_corrected = portfolio_values.copy()
            portfolio_values_corrected[-1] = final_value
        else:
            final_value = portfolio_values[-1]
            portfolio_values_corrected = portfolio_values

        ax1.plot(portfolio_values_corrected, color='darkblue', linewidth=2)
    ...
    #Se non abbiamo sufficienti dati di confronto, mostra un'alternativa più utile
    # MODIFICA QUI: Visualizza l'istogramma dei rendimenti giornalieri invece di un messaggio di errore
    if test_data.get('portfolio_value_log') is not None and len(test_data['portfolio_value_log']) > 1:
        portfolio_values = test_data['portfolio_value_log']
        # Calcola i rendimenti giornalieri come variazione percentuale
        daily_returns = np.diff(portfolio_values) / portfolio_values[:-1] * 100
        
        # Calcola il range corretto per i bin
        min_return = np.min(daily_returns)
        max_return = np.max(daily_returns)
        range_returns = max_return - min_return
        
        # Aumenta il numero di bin e assicurati che il range sia adeguato
        num_bins = 30  # Più bin per migliore granularità
        
        # Crea un array di bin con valori equidistanti e simmetrici intorno allo zero
        max_abs_return = max(abs(min_return), abs(max_return))
        bin_edges = np.linspace(-max_abs_return * 1.1, max_abs_return * 1.1, num_bins)
        
        # Visualizza l'istogramma con i bin migliorati
        n, bins, patches = ax6.hist(daily_returns, bins=bin_edges, color='navy', alpha=0.7, edgecolor='black')
        
        ax6.axvline(x=0, color='red', linestyle='--', alpha=0.5)
        ax6.set_title('Daily Returns Distribution (%)', fontsize=14)
        ax6.set_xlabel('Daily Return (%)', fontsize=12)
        ax6.set_ylabel('Frequency', fontsize=12)
        
        # Adatta gli assi per mostrare meglio la distribuzione
        ax6.set_xlim([min_return * 1.2, max_return * 1.2])  # Estendi un po' oltre i valori min/max
        
        # Aggiungi statistiche
        mean_return = np.mean(daily_returns)
        std_return = np.std(daily_returns)
        skew = ((daily_returns - mean_return) ** 3).mean() / std_return ** 3  # Asimmetria
        ...
        # Aggiungi una linea per la distribuzione normale ideale
        x = np.linspace(min_return, max_return, 100)
        y = np.exp(-(x - mean_return)**2 / (2 * std_return**2)) / (std_return * np.sqrt(2 * np.pi))
        y = y * len(daily_returns) * (bins[1] - bins[0])  # Scala per match con l'istogramma
        ax6.plot(x, y, 'r--', linewidth=1.5, label='Normal Distribution')
        ax6.legend()
    else:
        ax6.text(0.5, 0.5, "Insufficient data for comparison", 
                ha='center', va='center', fontsize=12)
        ax6.set_title('Performance Comparison', fontsize=14)
